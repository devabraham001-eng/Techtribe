...
import { ArrowLeft, Database as DatabaseIcon, ShieldAlert, BookOpen } from "lucide-react";
...
export default async function AdminLearningPage() {
  ...
  if (!user) {
    redirect("/login?next=/admin/learning");
  }
    ...
    return (
      <div className="mx-auto max-w-2xl px-6 pb-20 pt-10">
        <div className="rounded-lg border border-border bg-card p-8 text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">Only staff authors can manage learning content.</p>
          <Link
            href="/dashboard"
      ...
      <div className="mb-8 flex flex-col sm:flex-row items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Manage tracks, modules, and lessons. Signed in as {author?.name}.
          </p>
        </div>
      </div>
      <LearningAdmin />
